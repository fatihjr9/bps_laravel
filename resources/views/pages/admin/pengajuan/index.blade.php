    ...
    <div class="bg-slate-50 p-4 rounded-md border shadow-sm border-slate-200">
        <div class="flex flex-row items-center justify-between mb-2 border-slate-200">
            ...
            <div class="flex flex-row items-center">
                <h5 class="pr-4">Lama Kunjungan : {{ $pengajuan->tgl_mulai }} - {{ $pengajuan->tgl_selesai }} ( {{ $pengajuan->lama_hari }} hari )</h5>
                @if($pengajuan->status === "Menunggu Persetujuan")
                    <div class="w-fit bg-orange-50 text-orange-600 px-4 py-2 rounded-xl text-center">{{ $pengajuan->status }}</div>
                @elseif($pengajuan->status === "Ditolak")
                    <div class="w-fit bg-red-50 text-red-600 px-4 py-2 rounded-xl text-center">{{ $pengajuan->status }}</div>
                @else
                    <div class="w-fit bg-green-100 text-green-600 px-4 py-2 rounded-xl text-center">{{ $pengajuan->status }}</div>
                @endif
            </div>
        </div>
